...
项目切换器在左侧顶部，选择项目后整个sidebar内容只为当前项目服务。项目是第一层组织单元，工作区概念保留（与现有Paseo一致）。

```
有项目时的左侧栏：
┌─────────────────────────────────────┐
│  [下拉combo切换项目]  ← 显示当前项目名称 │
│   ▾ paseo                            │
├─────────────────────────────────────┤
│  📋 项目列表    ← 菜单项，点击→项目列表页 │
├─────────────────────────────────────┤
...
├─────────────────────────────────────┤
│  📂 工作区（git仓库）                  │
│   ├── repo-A  ← 完整workspace行信息    │
│   │    状态+分支+PR+checks+kebab       │
│   ├── repo-B  ← 微服务时多个仓库       │
│   └── ...                           │
├─────────────────────────────────────┤
│  🤖 Agent列表                        │
...
> 顶部不再有"Sessions"标签行（原Paseo的SidebarHeaderRow），只保留项目切换器combo。项目切换器仅在有项目时显示，无项目时隐藏并显示空状态引导。

### 1.2 核心规则

| 规则                     | 说明                                                                                                 | 变更 |
| ------------------------ | ---------------------------------------------------------------------------------------------------- | ---- |
| 项目切换器               | 顶部combo下拉选择当前项目，切换后整个sidebar内容更新为该项目数据；无项目时切换器隐藏，显示空状态引导 | 新增 |
| 工作区概念保留           | 工作区=项目根目录，git仓库在工作区区域显示（与现有Paseo一致）                                        | 变更 |
| Agent只能从项目创建      | 不存在"未归类"agent，所有agent必须属于一个项目                                                       | 变更 |
| Session cwd = 项目根目录 | 所有在项目下创建的agent，cwd都是项目rootDirectory                                                    | 不变 |
| 自由agent统一在项目下    | 自由agent显示在项目Agent列表，标注"自由"                                                             | 不变 |
| 中间区域动态切换         | 根据左侧选中内容切换中间区域内容                                                                     | 不变 |
| 不可编辑字段             | rootDirectory、id、createdAt创建后不可修改；git仓库URL不可修改只能删除重建                           | 新增 |

### 1.3 中间区域映射

| 左侧选中                 | 中间区域             | 右侧explorer          | 变更 |
| ------------------------ | -------------------- | --------------------- | ---- |
| 项目切换器切换项目       | 看板视图             | —                     | 新增 |
| 项目列表菜单             | 项目列表页           | —                     | 新增 |
| 任务列表菜单             | 看板视图             | —                     | 新增 |
| 看板中某任务（点击卡片） | 任务详情页           | —                     | 不变 |
| git仓库行（工作区）      | 保持当前内容不变     | 文件目录（该repo）    | 变更 |
| agent行                  | 对话流               | 文件目录（agent cwd） | 不变 |
| Add Project按钮          | 项目设置页（空表单） | —                     | 新增 |
| 项目列表页→编辑项目      | 项目设置页（填充）   | —                     | 新增 |
| 看板上方"分支管理"       | 分支管理视图         | —                     | 不变 |
| 看板上方"归档任务"       | 归档任务列表         | —                     | 不变 |

### 1.4 Agent归属规则

所有agent必须从项目创建，不存在"未归类"agent。agent创建时cwd自动设为项目的rootDirectory。
...
同一页面双模式：创建时为空表单，编辑时为填充表单。

**入口**：

| 模式 | 入口                                                            |
| ---- | --------------------------------------------------------------- |
| 创建 | Sidebar底部"Add Project"按钮（FolderPlus图标，与现有Paseo一致） |
| 编辑 | 项目列表页"编辑"按钮，或看板上方"项目设置"按钮                  |

**流程**：点击 → 中间区域全屏项目设置页 → 填写 → 创建模式：创建项目→sidebar更新→切换到该项目看板；编辑模式：更新项目→回到看板

**不可编辑字段**（编辑模式下）：

| 字段          | 显示方式              | 说明                            |
| ------------- | --------------------- | ------------------------------- |
| rootDirectory | 文字展示              | 创建后不可改，agent cwd始终绑定 |
| id            | 不显示在表单中        | 内部标识                        |
| createdAt     | 文字展示              | 只可查看                        |
| git仓库URL    | 展示+删除按钮，无编辑 | 只可删除后重新添加              |

**可编辑字段**：项目名称、项目描述、UAT分支名、生产分支名、禅道项目ID、CI/CD配置（全部）。

**表单字段**（16个，与原设计一致）：

| #   | 字段          | 表单控件                       | 必填                | 默认值 | 可编辑（编辑模式） |
| --- | ------------- | ------------------------------ | ------------------- | ------ | ------------------ |
| 1   | 项目名称      | 单行文本输入                   | 是                  | —      | 是                 |
| 2   | 项目描述      | 多行文本输入                   | 否                  | —      | 是                 |
| 3   | 项目根目录    | 路径选择器                     | 是                  | —      | **否**（文字展示） |
| 4   | Git仓库列表   | 动态添加列表（每条含URL+名称） | 否                  | 空     | 增删，URL不可改    |
| 5   | UAT分支名     | 单行文本输入                   | 否                  | —      | 是                 |
| 6   | 生产分支名    | 单行文本输入                   | 否                  | —      | 是                 |
| 7   | 禅道项目ID    | 单行文本输入                   | 否                  | —      | 是                 |
| 8   | CI/CD平台类型 | 下拉选择                       | 否                  | —      | 是                 |
| 9   | CI/CD URL     | 单行文本输入                   | 否（如8已选则必填） | —      | 是                 |
| 10  | 认证Token     | 单行文本输入                   | 否（如8已选则必填） | —      | 是                 |
| 11  | UAT Job名     | 单行文本输入                   | 否（如8已选则必填） | —      | 是                 |
| 12  | 生产Job名     | 单行文本输入                   | 否（如8已选则必填） | —      | 是                 |
| 13  | UAT自动触发   | 开关                           | 否                  | true   | 是                 |
| 14  | 生产自动触发  | 开关                           | 否                  | false  | 是                 |
...
**每个条目**：与现有Paseo workspace行完全一致——名称+状态指示器+分支名+PR badge+checks badge+kebab菜单。

**操作**：

- 点击条目 → 右侧explorer切换到该仓库文件树，中间区域保持当前内容不变
- 微服务项目显示多个仓库行
...
- 点击条目 → 中间区域切换到agent对话流，右侧explorer切换到agent cwd文件树
- "+"按钮 → 创建自由agent（cwd=项目rootDirectory，provider/model/mode从全局DefaultAgentConfig继承）

### 5.8 项目列表页

**入口**：Sidebar"项目列表"菜单项

**内容**：显示所有项目（排除已归档）的卡片/行
...
| -------- | ----------------- | ---------------- |
| 项目名称 | name              | 大标题           |
| 项目描述 | description       | 摘要文字（截断） |
| 根目录   | rootDirectory     | 路径文字         |
| 仓库数   | gitRepos.length   | 数字标签         |
| 任务数   | (按projectId统计) | 数字标签         |
| Agent数  | (按projectId统计) | 数字标签         |

**每个项目卡片操作按钮**：

| 操作 | 行为                                                |
| ---- | --------------------------------------------------- |
| 切换 | 项目切换器切换到该项目，中间显示看板                |
| 编辑 | 中间区域跳转到项目设置页（编辑模式）                |
| 归档 | 归档该项目（设archivedAt），级联归档其下任务和agent |

### 5.9 项目设置页

> 注意：5.1节已包含项目设置页的完整设计（共享创建/编辑模式），此节补充项目设置页在编辑模式下的入口。

**编辑模式入口**：项目列表页"编辑"按钮 + 看板上方"项目设置"按钮

**与5.1节的关系**：5.1节定义了项目设置页的完整表单（14字段+不可编辑字段规则），本节不再重复。
...
| 场景       | 提示          | 引导按钮         | Sidebar行为                              |
| ---------- | ------------- | ---------------- | ---------------------------------------- |
| 无项目     | "还没有项目"  | "新建项目"       | 项目切换器隐藏，中间+sidebar都显示空状态 |
| 看板无任务 | "还没有任务"  | "创建第一个任务" | Sidebar正常显示                          |
| 无agent    | "还没有Agent" | "创建Agent"      | Sidebar正常显示                          |
...
├── panels/
│   ├── project-settings-panel.tsx       # 项目设置页（创建/编辑共享）（新增，替代原create-project-panel）
│   ├── project-list-panel.tsx          # 项目列表页（新增）
...
├── components/
│   ├── sidebar-dev-platform-task-list.tsx  # Sidebar任务列表菜单项（需重构）
│   ├── sidebar-project-switcher.tsx        # Sidebar项目切换器combo（新增）
│   ├── sidebar-project-list-menu.tsx       # Sidebar项目列表菜单项（新增）
│   ├── sidebar-task-list-menu.tsx          # Sidebar任务列表菜单项（新增）
│   ├── sidebar-workspace-section.tsx       # Sidebar工作区区域（git仓库行，新增）
│   ├── sidebar-agent-list.tsx              # Sidebar Agent列表（新增）
...
| 集成点           | 说明                                                                                    |
| ---------------- | --------------------------------------------------------------------------------------- |
| Sidebar重构      | 去除顶部Sessions标签行，改为项目切换器combo→菜单项→工作区→Agent列表；无项目时切换器隐藏 |
| Agent创建限制    | 所有agent必须从项目创建，不存在未归类agent                                              |
...
| Explorer联动     | 点击sidebar git仓库时右侧explorer切换到该仓库（中间不变）                               |
| Add Project按钮  | 图标不变（FolderPlus），行为从文件选择器改为跳转项目设置页                              |
| 项目切换器       | 顶部combo下拉，切换项目后整个sidebar内容更新                                            |
...
| `packages/app/src/components/sidebar-dev-platform-task-list.tsx` | 需重构为菜单项样式（项目列表+任务列表菜单）                                                                                                                                                                                                                                                                     |
| `packages/app/src/components/left-sidebar.tsx`                   | 重构：从workspace→session改为：项目切换器→菜单项→工作区→Agent列表                                                                                                                                                                                                                                               |
...
| 文件                                                        | 内容                                                 |
| ----------------------------------------------------------- | ---------------------------------------------------- |
| `packages/app/src/panels/project-settings-panel.tsx`        | 项目设置页（创建/编辑共享，14字段+不可编辑字段规则） |
| `packages/app/src/panels/project-list-panel.tsx`            | 项目列表页（所有项目卡片+切换/编辑/归档）            |
...
| `packages/app/src/components/sidebar-project-switcher.tsx`  | Sidebar项目切换器combo（下拉选项目）                 |
| `packages/app/src/components/sidebar-project-list-menu.tsx` | Sidebar项目列表菜单项                                |
| `packages/app/src/components/sidebar-task-list-menu.tsx`    | Sidebar任务列表菜单项                                |
| `packages/app/src/components/sidebar-workspace-section.tsx` | Sidebar工作区区域（git仓库行，保持现有Paseo样式）    |
| `packages/app/src/components/sidebar-agent-list.tsx`        | Sidebar Agent列表（agent条目+"+"按钮）               |
...
| 文件                                                      | 修改内容                                                                                                        |
| --------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| `packages/app/src/stores/workspace-tabs-store/state.ts`   | WorkspaceTabTarget增加: project_settings/project_list/create_task/task_detail/task_activity/archived_tasks kind |
