...
Still show the summary table and the provenance line. Note: _"Could not render sample PNG (CLI unavailable). Save anyway on your OK."_ Do not block.
...
_"Image-based learning needs a vision-enabled model (Claude Sonnet or Opus). Re-run on such a model, or provide the `.drawio` source file instead."_
